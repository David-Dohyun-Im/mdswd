...
**Coding exercise – Query Week 4 from the CLI.** Navigate to `modern-software-dev-assignments/week4` and run:

```bash
claude -p "List every FastAPI route in backend/app/api.py." \
  --allowedTools ReadFile(backend/app/api.py)
```

Copy the response into `week4/writeup.md` to show you can call Claude Code outside the IDE.

## Step Quiz
**Questions**
1) What advantage does offering Claude Code across terminal, IDE, web, and SDKs provide?
2) Which assignment folder do you inspect when testing the CLI command above?
**Answers**
1) Developers can invoke the same agentic workflows wherever they are, reducing context switching between tooling surfaces.
2) `modern-software-dev-assignments/week4`.
...
**Coding exercise – Add an LLM endpoint.** Extend `week4/backend/app/api.py` with the new route described in the assignment instructions (it should call your Ollama-powered summarizer and return a typed response). Document the flow in `docs/API.md` so Claude Code or Cursor know how to invoke it.

## Step Quiz
**Questions**
1) Why does Claude Code support multiple “write code” modes (one-shot, sidekick, prototype)?
2) Which backend file do you modify to expose the Ollama-powered route?
**Answers**
...
**Coding exercise – Capture a workflow.** Add a section to `week4/docs/TASKS.md` describing a workflow your automation will follow:

```markdown
## Workflow — API Docs Refresh
1. Run `/tests`.
2. Execute `/docs-sync`.
3. Ask Claude to summarize changes and open a PR.
```

Reference this named workflow whenever you prompt Claude Code so it sticks to the agreed sequence.

## Step Quiz
**Questions**
1) What is the benefit of explicitly encoding workflow templates (explore → plan → confirm, etc.)?
2) Which file records the workflows you expect Claude to follow in Week 4?
**Answers**
...
**Coding exercise – Auto-generate README.** Complete Week 2’s TODO by asking Cursor (or Claude) to read the backend, frontend, and tests, then output a `README.md` describing setup, endpoints, and test commands. Keep the prompt and resulting file in the repo so future agents inherit the documentation.

## Step Quiz
**Questions**
1) What mindset shift does “ask not what the model can do for you” encourage?
2) Which deliverable demonstrates you can direct an agent to produce documentation from code?
**Answers**
1) To design products around reimagined workflows instead of bolting AI onto existing processes.
2) The generated `week2/README.md`.
...
**Coding exercise – Warp Drive prompt.** Follow the Week 5 instructions to create a Warp Drive workflow called `Run Tests`:
...
Save or export the workflow and paste the shared link into `week5/writeup.md`.

## Step Quiz
**Questions**
1) Why does Warp prioritize “five minutes to wow”?
2) Where do you document the Warp Drive prompt you built?
**Answers**
1) Early delight reduces onboarding friction and proves the agent adds value before users invest in deeper configuration.
2) `week5/writeup.md` (with the workflow link or exported definition).
...
**Coding exercise – Multi-agent sprint.** Week 5 challenges you to run simultaneous Warp agents. Create multiple `git worktree` checkouts (e.g., `week5/agent-a`, `week5/agent-b`) so each agent can tackle a different task from `docs/TASKS.md` without clobbering the others. Merge the branches once their tests pass.

## Step Quiz
**Questions**
1) What differentiates “YOLO mode” from lighter chat interactions?
2) Which Git feature helps isolate concurrent Warp agents?
**Answers**
1) YOLO mode gives the agent end-to-end autonomy to execute substantive tasks without constant human confirmation.
2) `git worktree`, as suggested in Week 5.
...
**Coding exercise – Bolt version folder.** For Week 8, create `week8/bolt-version/` and export your Bolt-generated project into it. Include a README with installation commands and note any manual patches you made after generation.

## Step Quiz
**Questions**
1) How does v0 change collaboration between PMs, designers, engineers, and founders?
2) Where do you store the Bolt-generated stack for Week 8?
**Answers**
...
2) In a folder like `week8/bolt-version/` with source plus a README.
...
**Coding exercise – Non-JS stack.** Spin up a second implementation (e.g., Django) under `week8/django-version/`:

```bash
django-admin startproject control_center week8/django-version
cd week8/django-version
python manage.py startapp notes
```

Build CRUD endpoints plus templates and document run instructions in `week8/django-version/README.md`.

## Step Quiz
**Questions**
1) Why do app builders constrain agents to established frameworks?
2) What language requirement does Week 8 impose across the three stacks?
**Answers**
1) Familiar stacks keep outputs maintainable and auditable, making it easier for humans to take over when necessary.
2) At least one version must use a non-JavaScript language for either frontend or backend.
...
**Coding exercise – Manual hardening notes.** After each Week 8 stack passes basic tests, add a manual hardening commit—for example, tighten serializers in Django or enforce input validation in Next.js. Document the change and reasoning in `week8/writeup.md` so evaluators can see how you stabilized the generated code.

## Step Quiz
**Questions**
1) What should teams be ready to do when AI-built apps break?
2) Where do you record the manual hardening steps for each Week 8 stack?
**Answers**
...
2) In `week8/writeup.md`, alongside the description for each version.
